...
export const DanOSGame = ({ onClose }: DanOSGameProps) => {
    ...
    return (
        <div ref={containerRef} className={`flex flex-col items-center justify-center bg-[#0a0a0a] text-white font-mono relative select-none transition-all duration-300 ${isFullscreen ? 'w-full h-full' : ''} ${isMobilePlaying ? 'fixed top-0 left-0 z-[100] w-full h-[100dvh] bg-black justify-center p-0' : 'w-full h-full md:w-auto md:h-full p-4'}`}>
            {/* Mobile Exit Button */}
            {isMobilePlaying && (
                <button
